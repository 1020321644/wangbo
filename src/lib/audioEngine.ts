  ...
  if (params.masterEnhance) {
    ...
    if (level === "advanced") {
      ...
      try {
        ...
        const audiosrUri = useModelStore.getState().getModelUri("audiosr");
        if (audiosrUri) {
          onEngine?.("audiosr");
          onProgress(0.02, "加载 AudioSR 模型...");
          const result = await runAudioSROnnx(sourceUri, outUri, audiosrUri, (p, label) => onProgress(p, label));
          if (result) {
            onProgress(1, "AudioSR 超分辨率完成");
            return outUri;
          }
        }
      } catch (e) {
        console.warn("[audioEngine] AudioSR ONNX 失败，降级 DeepFilterNet3:", e);
      }
    }

    if (level === "simple" || level === "advanced") {
      // 简单模式 / 困难模式降级：DeepFilterNet3 降噪
      try {
        const { useModelStore } = await import("@/store/modelStore");
        const df3Uri = useModelStore.getState().getModelUri("deepfilter3");
        if (df3Uri) {
          onEngine?.("deepfilternet");
          onProgress(0.02, "加载 DeepFilterNet3 模型...");
          const result = await runDeepFilterOnnx(sourceUri, outUri, df3Uri, (p, label) => onProgress(p, label));
          if (result) {
            onProgress(1, "DeepFilterNet3 降噪完成");
            return outUri;
          }
        }
      } catch (e) {
        console.warn("[audioEngine] DeepFilterNet3 ONNX 失败，降级 FFmpeg DSP:", e);
      }
    }
...
// ── DeepFilterNet3 ONNX 推理 ─────────────────────────────────────────────────
/**
 * 使用 DeepFilterNet3 ONNX 模型进行降噪（简单模式）
 *
 * 模型接口（标准 DeepFilterNet3 导出格式）：
 *   输入：
 *     - noisy_audio (float32 [1, T])   — 48kHz 单声道 PCM（帧对齐）
 *     - state       (float32 [1, ...]) — GRU 隐藏状态（首次为零）
 *   输出：
 *     - enhanced_audio (float32 [1, T])
 *     - new_state       (float32 [1, ...])
 *
 * 若模型路径无效或推理失败，返回 false（调用方降级）。
 */
async function runDeepFilterOnnx(
  inputUri: string,
  outputUri: string,
  modelUri: string,
  onProgress: (p: number, label: string) => void,
  ...
  try {
    ...
    onProgress(0.05, "DeepFilterNet3：读取音频...");

    // 读取 PCM 原始数据（假设输入为 WAV，先用 base64 解码）
    ...
    // 读取 PCM → Float32（单声道混音）
    const pcm = new Float32Array(nSamples / nCh);
    for (let i = 0; i < pcm.length; i++) {
      let sum = 0;
      for (let ch = 0; ch < nCh; ch++) {
        const off = dataOff + (i * nCh + ch) * bytesPS;
        sum += bps === 16 ? view.getInt16(off, true) / 32768.0
             : bps === 24 ? (() => { let v = raw[off] | (raw[off+1] << 8) | (raw[off+2] << 16); if (v & 0x800000) v |= ~0xFFFFFF; return v / 8388608.0; })()
             : view.getFloat32(off, true);
      }
      pcm[i] = sum / nCh;
    }

    // 重采样至 48kHz（DeepFilterNet3 固定要求）
    ...
    if (sr !== TARGET_SR) {
      ...
      for (let i = 0; i < outLen; i++) {
        ...
        resampled[i] = pcm[lo] + (pcm[hi] - pcm[lo]) * (src - lo);
      }
      inputPcm = resampled;
    }

    onProgress(0.2, "DeepFilterNet3：加载模型...");
    ...
    onProgress(0.3, "DeepFilterNet3：分块推理中...");

    // DeepFilterNet3 分块推理（帧长 480 samples @ 48kHz = 10ms）
    const FRAME = 480;
    const nFrames = Math.ceil(inputPcm.length / FRAME);
    const enhanced = new Float32Array(inputPcm.length);

    // 初始化 GRU 状态（若模型需要；若不需要则忽略）
    let state: Float32Array | null = null;

    for (let f = 0; f < nFrames; f++) {
      const start = f * FRAME;
      const end   = Math.min(start + FRAME, inputPcm.length);
      const frame = new Float32Array(FRAME); // 零填充
      frame.set(inputPcm.subarray(start, end));

      const inputTensor = new Tensor("float32", frame, [1, FRAME]);
      const feeds: Record<string, InstanceType<typeof Tensor>> = { noisy_audio: inputTensor };

      if (state !== null) {
        feeds["state"] = new Tensor("float32", state, [1, state.length]);
      }

      const result = await session.run(feeds);

      const outAudio = result["enhanced_audio"]?.data as Float32Array ?? frame;
      enhanced.set(outAudio.subarray(0, end - start), start);

      if (result["new_state"]) {
        state = result["new_state"].data as Float32Array;
      }

      if (f % 50 === 0) {
        onProgress(0.3 + 0.6 * (f / nFrames), `DeepFilterNet3：${Math.round((f / nFrames) * 100)}%`);
      }
    }

    // 编码输出 WAV 48kHz / 24-bit
    const outBytes = new Uint8Array(44 + enhanced.length * 3);
    const outView  = new DataView(outBytes.buffer);
    outBytes.set([82,73,70,70]); outView.setUint32(4, 36 + enhanced.length*3, true);
    outBytes.set([87,65,86,69,102,109,116,32], 8); outView.setUint32(16, 16, true);
    outView.setUint16(20, 1, true); outView.setUint16(22, 1, true);
    outView.setUint32(24, TARGET_SR, true); outView.setUint32(28, TARGET_SR*3, true);
    outView.setUint16(32, 3, true); outView.setUint16(34, 24, true);
    outBytes.set([100,97,116,97], 36); outView.setUint32(40, enhanced.length*3, true);
    for (let i = 0; i < enhanced.length; i++) {
      let v = Math.max(-1, Math.min(1, enhanced[i]));
      let s = Math.round(v * 8388607);
      if (s < 0) s += 0x1000000;
      outView.setUint8(44 + i*3,     s & 0xFF);
      outView.setUint8(44 + i*3 + 1, (s >> 8) & 0xFF);
      outView.setUint8(44 + i*3 + 2, (s >> 16) & 0xFF);
    }

    const b64Out = btoa(String.fromCharCode(...Array.from(outBytes)));
    await FileSystemLeg.writeAsStringAsync(outputUri, b64Out, { encoding: "base64" });

    onProgress(0.98, "DeepFilterNet3：写入完成");
    console.log("[audioEngine] ✅ DeepFilterNet3 推理成功");
    return true;
  } catch (e) {
    console.error("[audioEngine] DeepFilterNet3 推理失败:", e);
    return false;
  }
}
...
// ── AudioSR ONNX 推理 ─────────────────────────────────────────────────────────
/**
 * 使用 AudioSR ONNX 模型进行超分辨率重建（困难模式）
 *
 * 模型接口（标准 AudioSR ONNX 导出格式）：
 *   输入：
 *     - low_res_audio (float32 [1, 1, T])  — 低采样率 PCM（单声道）
 *   输出：
 *     - high_res_audio (float32 [1, 1, T]) — 超分辨率 PCM
 *
 * 若模型路径无效或推理失败，返回 false（调用方降级）。
 */
async function runAudioSROnnx(
  inputUri: string,
  outputUri: string,
  modelUri: string,
  onProgress: (p: number, label: string) => void,
  ...
  try {
    ...
    onProgress(0.1, "AudioSR：加载模型（可能需要较长时间）...");
    ...
    onProgress(0.2, "AudioSR：分块超分推理中...");

    // AudioSR 分块推理（chunk size 65536 samples）
    const CHUNK    = 65536;
    const OVERLAP  = 4096;
    const nChunks  = Math.ceil(pcm.length / (CHUNK - OVERLAP));
    const enhanced = new Float32Array(pcm.length * 2); // 假设 2x 上采样

    let outOffset = 0;
    for (let c = 0; c < nChunks; c++) {
      const start = c * (CHUNK - OVERLAP);
      const end   = Math.min(start + CHUNK, pcm.length);
      const chunk = new Float32Array(CHUNK);
      chunk.set(pcm.subarray(start, end));

      const inputTensor = new Tensor("float32", chunk, [1, 1, CHUNK]);
      const result      = await session.run({ low_res_audio: inputTensor });
      const outChunk    = result["high_res_audio"]?.data as Float32Array ?? chunk;

      const copyLen = Math.min(outChunk.length, enhanced.length - outOffset);
      enhanced.set(outChunk.subarray(0, copyLen), outOffset);
      outOffset += copyLen - (OVERLAP * 2);

      onProgress(0.2 + 0.7 * (c / nChunks), `AudioSR：${Math.round((c / nChunks) * 100)}%`);
    }

    const actualOut = enhanced.subarray(0, outOffset);
    const OUT_SR    = sr * 2; // 2x 超分辨率

    // 编码输出 WAV 48kHz / 24-bit
    const finalSr  = Math.min(OUT_SR, 48000);
    const outBytes = new Uint8Array(44 + actualOut.length * 3);
    const outView  = new DataView(outBytes.buffer);
    outBytes.set([82,73,70,70]); outView.setUint32(4, 36 + actualOut.length*3, true);
    outBytes.set([87,65,86,69,102,109,116,32], 8); outView.setUint32(16, 16, true);
    outView.setUint16(20, 1, true); outView.setUint16(22, 1, true);
    outView.setUint32(24, finalSr, true); outView.setUint32(28, finalSr*3, true);
    outView.setUint16(32, 3, true); outView.setUint16(34, 24, true);
    outBytes.set([100,97,116,97], 36); outView.setUint32(40, actualOut.length*3, true);
    for (let i = 0; i < actualOut.length; i++) {
      let v = Math.max(-1, Math.min(1, actualOut[i]));
      let s = Math.round(v * 8388607);
      if (s < 0) s += 0x1000000;
      outView.setUint8(44 + i*3,     s & 0xFF);
      outView.setUint8(44 + i*3 + 1, (s >> 8) & 0xFF);
    ...
    onProgress(0.98, "AudioSR：写入完成");
    console.log("[audioEngine] ✅ AudioSR 推理成功");
    return true;
  } catch (e) {
    console.error("[audioEngine] AudioSR 推理失败:", e);
    return false;
  }
}
