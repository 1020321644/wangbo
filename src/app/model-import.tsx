 ...
 * AI 模型管理页面
 *
 * 用户可从手机外部导入 .onnx 模型文件，供本地 AI 处理使用：
 *   - deepfilter3：DeepFilterNet3 降噪（简单模式）
 *   - audiosr：AudioSR 超分辨率（困难模式）
 *
 * 导入的文件会被复制到应用私有目录持久保存，路径记录在 modelStore 中。
 * assets/ 中不包含任何模型文件。
 */
...
import {
  ChevronLeft, Upload, Trash2, Cpu, CheckCircle2, AlertCircle,
...
const MODEL_SLOTS: ModelSlot[] = [
  {
    ...
    desc: "AI 降噪模型 · 简单模式（母带级品质提升）",
    sizeHint: "约 8.6 MB",
  },
  {
    id: "audiosr",
    label: "AudioSR",
    desc: "AI 超分辨率模型 · 困难模式（母带级品质提升）",
    sizeHint: "约 20 ~ 100 MB",
  },
];
      ...
      >
        ...
        >
          ...
          <Text style={{ fontFamily: "monospace", fontSize: 9, color: C.muted, lineHeight: 14 }}>
            从手机本地导入 .onnx 模型文件，用于「母带级品质提升」的本地 AI 处理。
            模型文件不会被内置，仅保存在本机。未导入模型时，相关功能将自动降级为 FFmpeg DSP 处理。
          </Text>
        </View>
            ...
            >
              ...
              >
                ...
                {imported ? (
                  ...
                  <Text style={{ fontFamily: "monospace", fontSize: 9, color: C.muted }}>未导入</Text>
                )}
              </View>
