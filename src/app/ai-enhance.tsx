...
import {
  ...
  Volume2, Sparkles, Gauge, RefreshCw, Cpu, Zap, Cloud, CloudOff,
...
export default function AIEnhanceScreen() {
  ...
  const startProcess = useCallback(async () => {
    ...
    try {
      // ── 1. 本地优先处理（FFmpeg + ONNX，离线可用）──────────────────────
      let outUri = await runConvert(
        file.uri,
        file.name,
        "WAV",
        {
      ...
      let outName = file.name.replace(/\.[^.]+$/, "") + `_enhanced_${mode}.wav`;
      let note = "本地处理完成";

      // ── 2. 云端增强（可选，仅困难模式 + 用户开启）──────────────────────
      // 隐私：仅上传本地处理后的临时文件，不上传原始文件
      if (mode === "advanced" && cloudEnhance) {
        targetProgressRef.current = 100;
        setPhase("云端增强中… 预计 25 秒");
        const cloud = await cloudEnhanceAudio(outUri, (p, label) => {
          setPhase(label);
          targetProgressRef.current = Math.round(p * 100);
        });
        if (cloud.ok && cloud.uri) {
          outUri = cloud.uri;
          outName = file.name.replace(/\.[^.]+$/, "") + `_enhanced_${mode}_cloud.wav`;
          note = "本地处理完成，已额外应用云端增强";
        } else {
          // 云端失败：保留本地结果
          note = `本地处理完成，云端增强失败（${cloud.error ?? "繁忙"}）`;
        }
      }
  ...
  }, [file, mode, addFiles, cloudEnhance]);
    ...
    <View className="flex-1 bg-background">
      ...
      <KeyboardAvoidingView behavior={process.env.EXPO_OS === "ios" ? "padding" : undefined} className="flex-1">
        ...
        >
          ...
          {mode === "advanced" ? (
            <View
              className="flex-row items-center gap-2 border p-3"
              style={{ borderColor: cloudEnhance ? C.orange : C.border, backgroundColor: cloudEnhance ? `${C.orange}12` : "transparent" }}
            >
              {cloudEnhance ? <Cloud size={16} color={C.orange} strokeWidth={1.5} /> : <CloudOff size={16} color={C.muted} strokeWidth={1.5} />}
              <Text className="flex-1 font-mono text-[10px] leading-4" style={{ color: cloudEnhance ? C.orange : C.muted }}>
                {cloudEnhance
                  ? "云端增强已开启：本地处理完成后将自动调用外网 AI 二次优化（仅上传处理结果）"
                  : "云端增强已关闭：仅本地处理。可在「设置」中开启云端增强"}
              </Text>
            </View>
          ) : null}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {result ? `${successNote}\n${result.name}（${formatFileSize(result.size)}）已保存，可在播放器查看。` : "音频已增强并保存。"}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
